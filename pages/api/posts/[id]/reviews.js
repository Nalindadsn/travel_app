...
const postHandler = async (req, res) => {
  ...
  if (post) {
    ...
    if (existReview) {
      ...
      post.numReviewsOne = post.reviews.filter((r) => r.rating == 1).length;
      post.numReviewsTwo = post.reviews.filter((r) => r.rating == 2).length;
      post.numReviewsThree = post.reviews.filter((r) => r.rating == 3).length;
      post.numReviewsFour = post.reviews.filter((r) => r.rating == 4).length;
      post.numReviewsFive = post.reviews.filter((r) => r.rating == 5).length;
